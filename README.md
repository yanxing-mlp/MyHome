...
- **身份怎么传**这条见下面"账号体系"一节。2026-09-22 起身份凭据是**服务端签发的 HMAC Bearer token**（`Authorization: Bearer <token>`），不再是可伪造的 `X-User-Id` 请求头——那个头已整条下线、**没有任何回退**（留回退等于把越权口子重新打开）。判权限的口径不变：创建行的接口都要求登录身份（要往 `creator_id` 里写东西的那 9 个，含 2026-09-23 的购物车插入），账号管理那 **4** 个（列表 / 新建 / **改角色** / 删除）还额外要 ADMIN；`/me` 与 B 端个人中心那三条也只要"有身份"、不判角色；v14 相册个人列表无身份 401，B/C 裸 ID 读写均校验分区/属主，跨区/跨属主 404，详见接口表。其他业务域尚未全面做资源属主保护。**ADMIN 对别人账号的全部权力是"加人""删人""定谁是超管"三件事**：除了 `role` 那一列，改不了任何人的昵称、手机号、口令，也看不到口令
...
- **`creator_id` 只存 id，不存昵称、也不 join**：八张表（`file_object`、`album_group`、`album_image`、`recipe`、`recipe_category`、`recipe_order`、`vault_account`、**`recipe_cart_item`**）加的都是 `creator_id`，**除购物车外都可空**。前七张只在插入那一行写它，改量/改做法走 update 分支不重写（与 `recipe_order` 的"继续加菜不改下单人"同一口径）。**购物车是例外（V319 加可空列 → V505 收口成一人一菜一行）**：`creator_id` 现为 `NOT NULL` 且进唯一键 `uk_recipe_creator(recipe_id, creator_id)`，同一道菜不同加购人各占一行；`setItem`（改量与减到 0 删行两个分支）和"再来一单"都要先取身份、只动"调用人自己那道菜的那一行"，同菜别人的行不受影响，`clearCart` 仍是整辆车。昵称由两端各自拿 `{id,name}` 字典在前端解析（B 端 `GET /api/b/user/options`、C 端 `GET /api/c/user/options`，**两条路径、同一个 `options()` 方法、同一个 `UserOptionVO`**）——相册/菜谱/文件/密码本四个域之间唯一的公共依赖是 `fh-common`，为了一列显示文字凭空开出四条跨域 join 不值当。字典里查不到（账号已软删）显示"已删除账号"；`creator_id` 为 NULL 就整段不渲染，不放占位符。
...
|  | `GET/PUT/DELETE /api/c/recipe/cart` | 全家共享购物车（**一人一菜一行**，V505：唯一键 `(recipe_id, creator_id)`，同菜不同加购人各占一行），三条均返回 `{version,items}`。PUT 必传 `{version,recipeId,qty,practices?}`，改量/移除都只动**调用人自己**那道菜的那一行、`qty<=0` 移除、做法整体覆盖；DELETE 必传 `?version=...` 清整辆车。旧版写入返回 400，不自动重放。B 端没有购物车接口 |
