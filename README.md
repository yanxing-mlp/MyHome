...
- **`creator_id` 只存 id，不存昵称、也不 join**：八张表（`file_object`、`album_group`、`album_image`、`recipe`、`recipe_category`、`recipe_order`、`vault_account`、**`recipe_cart_item`**（2026-09-23 加：加购人，`V319` 加列、存量由 user 域 `V504` 洗成大宝；**只有插入那一行写它**，别人改量/改做法走 update 分支不重写，与 `recipe_order` 的"继续加菜不改下单人"同一口径））加的都是可空 `creator_id`。昵称由两端各自拿 `{id,name}` 字典在前端解析（B 端 `GET /api/b/user/options`、C 端 `GET /api/c/user/options`，**两条路径、同一个 `options()` 方法、同一个 `UserOptionVO`**）——相册/菜谱/文件/密码本四个域之间唯一的公共依赖是 `fh-common`，为了一列显示文字凭空开出四条跨域 join 不值当。字典里查不到（账号已软删）显示"已删除账号"；`creator_id` 为 NULL 就整段不渲染，不放占位符。
